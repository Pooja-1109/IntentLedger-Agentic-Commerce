  ...
  return (
    <div className="space-y-8 animate-fadeIn max-w-5xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-200">
        ...
        {/* Live Telemetry Pill */}
        <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-lg bg-white border border-slate-200 text-xs self-start md:self-auto shadow-2xs">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 text-xs">
        <div className="fintech-card p-4 space-y-1">
          ...
          <span className="font-extrabold text-emerald-700 flex items-center gap-1 text-sm">
            <CheckCircle2 className="w-4 h-4 text-emerald-600" /> Verified
          </span>
          <span className="text-[11px] text-slate-600 block font-mono font-medium">SHA-256 Snapshot</span>
        </div>
        ...
        <div className="fintech-card p-4 space-y-1">
          ...
          <span className="text-[11px] text-slate-600 block font-mono font-medium">10m TTL Bound</span>
        </div>
        ...
        <div className="fintech-card p-4 space-y-1">
          ...
          <span className="text-[11px] text-slate-600 block font-mono font-medium">Timing-Safe</span>
        </div>

        <div className="fintech-card p-4 space-y-1">
          <span className="text-[10px] font-bold text-slate-500 uppercase block">Payment Rail</span>
          <span className="font-extrabold text-amber-800 flex items-center gap-1 text-sm">
            <CheckCircle2 className="w-4 h-4 text-amber-600" /> Test Mode
          </span>
          <span className="text-[11px] text-slate-600 block font-mono font-medium">Zero Exposure</span>
        </div>
        ...
        <div className="fintech-card p-4 space-y-1">
          ...
          <span className="text-[11px] text-slate-600 block font-mono font-medium">HTTP 405 Guard</span>
        </div>
      </div>

      {/* Hero Security Invariant Banner (High-Impact Dark Navy Banner with Maximum Contrast) */}
      <div className="fintech-card-dark p-6 md:p-8 space-y-3 border-l-4 border-l-blue-500 bg-slate-900 text-white shadow-lg">
        ...
        <p className="text-slate-200 text-sm leading-relaxed max-w-3xl font-normal">
      ...
      <div className="fintech-card p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
          <div className="p-4 rounded-lg bg-slate-50 border border-slate-200">
            ...
            <div className="text-3xl font-extrabold text-slate-900 tabular-nums mt-1">
              {loading ? "..." : summary?.decisionsEvaluated ?? 0}
            </div>
            <span className="text-[11px] text-slate-600 mt-1 block font-medium">Deterministic policy runs</span>
          </div>

          <div className="p-4 rounded-lg bg-rose-50 border border-rose-200">
            <span className="text-[10px] font-bold text-rose-800 uppercase block">Violations Blocked</span>
            <div className="text-3xl font-extrabold text-rose-600 tabular-nums mt-1">
              {loading ? "..." : summary?.blockedActions ?? 0}
            </div>
            <span className="text-[11px] text-slate-600 mt-1 block font-medium">Halted at policy gate</span>
          </div>

          <div className="p-4 rounded-lg bg-amber-50 border border-amber-200">
            <span className="text-[10px] font-bold text-amber-800 uppercase block">Intent Drift Flagged</span>
            <div className="text-3xl font-extrabold text-amber-700 tabular-nums mt-1">
              {loading ? "..." : summary?.driftDetected ?? 0}
            </div>
            <span className="text-[11px] text-slate-600 mt-1 block font-medium">Parameters deviated</span>
          </div>

          <div className="p-4 rounded-lg bg-emerald-50 border border-emerald-200">
            <span className="text-[10px] font-bold text-emerald-800 uppercase block">Verified Settlements</span>
            <div className="text-3xl font-extrabold text-emerald-700 tabular-nums mt-1">
              {loading ? "..." : summary?.successfulPayments ?? 0}
            </div>
            <span className="text-[11px] text-slate-600 mt-1 block font-medium">Settled with audit trace</span>
          </div>
        </div>
      </div>

      {/* Cryptographic Tampering Attack Proof */}
      <div className="fintech-card p-6 space-y-4 border-rose-200 bg-rose-50/20">
        <div className="flex items-center justify-between pb-3 border-b border-slate-200">
          <div className="flex items-center gap-2 text-rose-800 font-bold text-xs uppercase tracking-wider">
            <ShieldAlert className="w-5 h-5 text-rose-600" />
            <span>Cryptographic Proof: Hero Tampering Attack Defense</span>
          </div>
          <span className="text-xs font-mono uppercase px-2.5 py-0.5 rounded bg-rose-100 text-rose-900 border border-rose-300 font-bold">
            Scenario D Invariant
          </span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-5 gap-3 text-xs">
          <div className="p-3.5 rounded-lg bg-white border border-slate-200 space-y-1 shadow-2xs">
            <span className="text-[10px] font-bold text-slate-500 uppercase block">1. User Approval</span>
            <div className="font-extrabold text-emerald-700 text-sm">₹3,499 INR</div>
            <div className="text-xs text-slate-700 font-medium">Nike Pegasus Running Shoes</div>
          </div>

          <div className="p-3.5 rounded-lg bg-white border border-slate-200 space-y-1 shadow-2xs">
            <span className="text-[10px] font-bold text-slate-500 uppercase block">2. Approval Token</span>
            <div className="font-mono text-xs text-blue-800 truncate font-bold">sha256(snap_3499_nike)</div>
            <div className="text-xs text-slate-700 font-medium">10-Minute TTL Bound</div>
          </div>

          <div className="p-3.5 rounded-lg bg-white border border-rose-300 space-y-1 shadow-2xs">
            <span className="text-[10px] font-bold text-rose-700 uppercase block">3. Rogue Agent Alteration</span>
            <div className="font-extrabold text-rose-600 text-sm">₹7,999 INR</div>
            <div className="text-xs text-slate-700 font-medium">Nike Vaporfly Elite Pro</div>
          </div>

          <div className="p-3.5 rounded-lg bg-white border border-rose-300 space-y-1 shadow-2xs">
            <span className="text-[10px] font-bold text-rose-700 uppercase block">4. IntentLedger Verdict</span>
            <div className="font-extrabold text-rose-700 text-sm">403 MISMATCH</div>
            <div className="text-xs text-rose-600 font-mono truncate font-bold">APPROVAL_CONTEXT_MISMATCH</div>
          </div>

          <div className="p-3.5 rounded-lg bg-white border border-emerald-300 space-y-1 shadow-2xs">
            <span className="text-[10px] font-bold text-emerald-700 uppercase block">5. Payment Rail State</span>
            <div className="font-extrabold text-emerald-800 text-sm">NOT CREATED</div>
            <div className="text-xs text-slate-700 font-medium">Zero Financial Exposure</div>
          </div>
        </div>

        <div className="p-3.5 rounded-lg bg-white border border-slate-200 text-xs text-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-2xs font-medium">
      ...
      <div className="fintech-card p-6 space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-slate-200">
          ...
          <span className="text-[11px] font-mono text-emerald-900 bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-200 font-bold">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
          <div>
            <label className="block text-[11px] font-bold text-slate-800 mb-1">Razorpay Order ID</label>
            <input
              type="text"
              value={orderId}
              onChange={(e) => setOrderId(e.target.value)}
              className="w-full rounded-lg bg-slate-50 border border-slate-300 p-2.5 text-slate-900 font-mono text-xs focus:outline-none focus:border-blue-500 shadow-2xs font-semibold"
            />
          </div>

          <div>
            <label className="block text-[11px] font-bold text-slate-800 mb-1">Razorpay Payment ID</label>
            <input
              type="text"
              value={paymentId}
          ...
          <div>
            <label className="block text-[11px] font-bold text-slate-800 mb-1">Webhook / Key Secret</label>
            <input
              type="text"
              value={secretKey}
        ...
        <div className="p-4 rounded-lg bg-slate-50 border border-slate-200 space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold text-slate-700 uppercase">Generated Signature Hash (Hex):</span>
            <button
              onClick={() => {
          ...
          <div className="font-mono text-xs text-blue-950 font-bold break-all bg-white p-3 rounded-lg border border-slate-300 shadow-2xs">
        ...
        <div className="p-4 rounded-lg bg-slate-50 border border-slate-200 space-y-3">
          ...
          <div className="flex flex-col sm:flex-row gap-2.5">
            <input
              ...
              className="flex-1 rounded-lg bg-white border border-slate-300 p-2.5 text-slate-900 font-mono text-xs focus:outline-none focus:border-blue-500 shadow-2xs font-semibold"
            />
            <button
              onClick={handleVerify}
              className="px-6 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold transition-all shrink-0 shadow-sm"
            >
              Verify Signature
            </button>
          </div>
          ...
          {verificationResult !== null && (
            <div
              className={`p-3.5 rounded-lg border text-xs font-bold flex items-center gap-2 ${
                verificationResult
