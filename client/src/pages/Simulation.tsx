...
export const SimulationPage: React.FC = () => {
  ...
  const [proposalCurrency] = useState<string>("INR");
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      {/* 2-Column Split: Proposal Input Form vs Decision Verdict Panel */}
            ...
            >
              {intents.map((intent) => (
                <option key={intent.id} value={intent.id}>
                  {intent.category.toUpperCase()} • Limit ₹{intent.constraints.maxAmount?.toLocaleString()} — "{intent.rawText.substring(0, 30)}..."
                </option>
              ))}
            </select>
          </div>
